...
from colibrie.utils import RangeMap, closest_value
...
def get_vertical_fragmented_lines(lines_fragmented):
    return [
        line
        for line in lines_fragmented
        if math.isclose(line[0].x, line[1].x, abs_tol=5) and line[0].y != line[1].y
    ]
...
def get_horizontal_fragmented_lines(lines_fragmented):
    return [
        line
        for line in lines_fragmented
        if math.isclose(line[0].y, line[1].y, abs_tol=5) and line[0].x != line[1].x
    ]


def generate_missing_horizontal_lines(horizontal_lines, vertical_lines, distinct_y_lst):
...
def generate_missing_vertical_lines(horizontal_lines, vertical_lines, distinct_x_lst):
    if not horizontal_lines or not vertical_lines:
...
def remove_overlaping_vertical_lines(vertical_lines):
    ...
    for line in vertical_lines:
        ...
        if line[0].x != current_x:
            ...
            merged_vertical_lines += [
                (Point(current_x, interval.start), Point(current_x, interval.end))
...
def remove_overlaping_horizontal_lines(horizontal_lines):
    ...
    for line in horizontal_lines:
        ...
        if line[0].y != current_y:
            ...
            merged_horizontal_lines += [
                (Point(interval.start, current_y), Point(interval.end, current_y))
                for interval in merged_interval
            ]
            group = [line]
            current_y = line[0].y
...
def normalize_vertical_lines(vertical_lines, horizontal_lines):
    ...
    for vertical_line in vertical_lines:
        ...
        vertical_line[0].y = (
            closest_y0
            if math.isclose(vertical_line[0].y, closest_y0, abs_tol=3)
            else vertical_line[0].y
        )
        vertical_line[1].y = (
            closest_y1
            if math.isclose(vertical_line[1].y, closest_y1, abs_tol=3)
            else vertical_line[1].y
        )
...
def normalize_horizontal_lines(vertical_lines, horizontal_lines):
    ...
    for horizontal_line in horizontal_lines:
        ...
        horizontal_line[0].x = (
            closest_x0
            if math.isclose(horizontal_line[0].x, closest_x0, abs_tol=3)
            else horizontal_line[0].x
        )
        horizontal_line[1].x = (
            closest_x1
            if math.isclose(horizontal_line[1].x, closest_x1, abs_tol=3)
            else horizontal_line[1].x
        )
